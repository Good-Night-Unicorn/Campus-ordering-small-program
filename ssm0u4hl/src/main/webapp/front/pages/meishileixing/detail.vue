...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','meishileixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','meishileixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('meishileixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussmeishileixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('meishileixing', this.detail);
				this.$utils.msg('审核成功');
